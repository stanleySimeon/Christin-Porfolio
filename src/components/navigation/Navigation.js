...
function Navigation() {
  ...
  return (
    <nav id="navigation" className="w-full h-12 md:h-20 flex justify-between items-center fixed py-8 px-3 sm:px-8 md:px-16 lg:px-24 xl:px-36 2xl:px-48 bg-christin-white z-50">
      ...
      <FontAwesomeIcon icon={faBars} className="md:hidden text-3xl text-christin rounded-none" id="toggle-button" onClick={toggleMenu} />
      <ul className="navbar w-full sm:w-1/2 hidden md:flex absolute top-0 left-0 md:left-0 right-0 sm:left-1/2 sm:right-0 px-3 sm:px-8 md:px-0 h-screen md:h-auto md:relative md:w-auto text-christin-white md:text-christin bg-christin bg-opacity-95 md:bg-transparent" id="menu">
        ...
        <span className="w-full font-montserrat md:text-xs lg:text-sm xl:text-md flex flex-col md:flex-row justify-center md:justify-end md:items-center sm:items-start sm:pl-8 space-y-8 md:space-y-0 md:space-x-6 lg:space-x-8 xl:space-x-10 2xl:space-x-12 whitespace-nowrap cursor-pointer py-12 md:py-0">
          <li>
            <Link
              to="about"
              spy
